...
def func(message):
    ...
    if message.text == "📸 Отчет":
        ...
        cur = base.cursor()
        create_table = 'CREATE TABLE IF NOT EXISTS {table} (Время заказа, Описание заказа, Фотография_id)'
        base.execute(create_table.format(table=f''))
        base.commit()
        cur.execute(f'INSERT INTO (?) VALUES(?, ?, ?)', (str(message.chat.id), otchet_time, message_text, str(photo)))
        base.commit()
        ...
        while count < len(info_id_list):
            ...
            count += 1
        if true == 1:
            bot.send_message(message.chat.id, text='Вы уже зарегестрированы', reply_markup=markup_main)
        else:
